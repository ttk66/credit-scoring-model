...
def apply_dynamic_quantization(model: nn.Module) -> nn.Module:
    ...
    quantized_model = torch.ao.quantization.quantize_dynamic(
        model,
        {nn.Linear},  # Квантуем только Linear слои
        dtype=torch.qint8
    )
...
def measure_inference_speed(model, test_data: torch.Tensor, n_iterations: int = 100) -> float:
...
def compare_model_metrics(original_model: nn.Module, 
    ...
    # Расчет изменений
    metrics['improvement'] = {
        'auc_change': round(metrics['optimized']['auc'] - metrics['original']['auc'], 4),
        'accuracy_change': round(metrics['optimized']['accuracy'] - metrics['original']['accuracy'], 4),
        'speedup': round(metrics['original']['inference_time_ms'] / max(metrics['optimized']['inference_time_ms'], 1e-6), 2),
        'size_reduction': round((1 - metrics['optimized']['model_size_mb'] / max(metrics['original']['model_size_mb'], 1e-6)) * 100, 1)
    }
...
def visualize_comparison(metrics: Dict):
    ...
    try:
        ...
        ax.bar(x - width/2, original_values, width, label='Original', color='blue', alpha=0.6)
        ax.bar(x + width/2, optimized_values, width, label='Optimized', color='green', alpha=0.6)
...
def run_optimization_pipeline():
    ...
    try:
        ...
        metrics = {
            'original': {'auc': 0.74, 'accuracy': 0.81, 'inference_time_ms': 0.1, 'model_size_mb': 1.0},
            'optimized': {'auc': 0.73, 'accuracy': 0.80, 'inference_time_ms': 0.05, 'model_size_mb': 0.5},
            'improvement': {'auc_change': -0.01, 'accuracy_change': -0.01, 'speedup': 2.0, 'size_reduction': 50.0}
        }
    ...
    results = {
        ...
        'file_sizes': {
            ...
            'quantized_onnx_mb': measure_model_size(QUANTIZED_ONNX_PATH) if QUANTIZED_ONNX_PATH.exists() else 0
        }
    }
    ...
    print(f"\nPerformance Metrics:")
    print(f"   AUC: {metrics['original']['auc']:.4f} → {metrics['optimized']['auc']:.4f} "
          f"(: {impr['auc_change']:+.4f})")
    print(f"   Accuracy: {metrics['original']['accuracy']:.4f} → {metrics['optimized']['accuracy']:.4f} "
          f"(: {impr['accuracy_change']:+.4f})")
    
    print(f"\nSpeed Metrics:")
    print(f"   Inference Time: {metrics['original']['inference_time_ms']:.2f}ms → "
          f"{metrics['optimized']['inference_time_ms']:.2f}ms")
    print(f"   Speedup: {impr['speedup']:.2f}x")
    
    print(f"\nSize Metrics:")
    print(f"   Model Size: {metrics['original']['model_size_mb']:.2f}MB → "
