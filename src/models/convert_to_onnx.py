...
def convert_to_onnx(pytorch_model: torch.nn.Module, 
                    input_size: int,
                    onnx_path: Path,
...
def compare_performance(pytorch_model: torch.nn.Module,
                       onnx_session: ort.InferenceSession,
                       test_data: np.ndarray,
...
def run_complete_pipeline():
    ...
    results = {
        ...
        'model_info': {
            ...
            'file_size_mb': ONNX_PATH.stat().st_size / (1024 * 1024) if ONNX_PATH.exists() else 0
        }
    }
    ...
    try:
        ...
        onnx_pred = onnx_session.run(None, {onnx_session.get_inputs()[0].name: sample_input})[0]
...
def simple_convert():
    ...
    try:
        ...
        # ONNX
        ort_out = ort_session.run(None, {'input': test_input.numpy().astype(np.float32)})[0]
