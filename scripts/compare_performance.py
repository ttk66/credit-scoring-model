...
class PerformanceComparator:
    ...
    def benchmark_random_forest(self, model, test_data: np.ndarray, batch_size: int) -> Dict:
    ...
    def measure_system_resources(self) -> Dict:
        ...
        if torch.cuda.is_available():
            ...
            resources['gpu_memory_total_mb'] = torch.cuda.get_device_properties(0).total_memory / (1024 * 1024)
        
        return resources
    
    def benchmark_pytorch(self, model, test_data: np.ndarray, batch_size: int, device: str = 'cpu') -> Dict:
    ...
    def run_benchmarks(self):
        ...
        for device in self.devices:
            ...
            for batch_size in self.batch_sizes:
                ...
                for model_name, model in self.models.items():
                    ...
                    if device == 'cuda' and model_name not in ['pytorch_nn', 'optimized_nn']:
                        continue
                    ...
                    try:
                        if model_name == 'random_forest':
                            results = self.benchmark_random_forest(model, test_data, batch_size)
                        elif 'onnx' in model_name:
                            results = self.benchmark_onnx(model, test_data, batch_size)
                        else:
                            # Для PyTorch моделей указываем устройство
                            results = self.benchmark_pytorch(model, test_data, batch_size, device)
                        ...
                        if device == 'cuda' and 'gpu_memory_mb' in results:
                            print(f"{results['predictions_per_second']:.1f} pred/s (GPU Mem: {results['gpu_memory_mb']:.1f} MB)")
                        else:
        ...
        print(f"  Memory: {after_resources['memory_used_mb']:.1f} MB / {after_resources['memory_total_mb']:.1f} MB")
        ...
        try:
            ...
            for model_name in ['pytorch_nn', 'optimized_nn']:
                ...
                for batch_size in self.batch_sizes:
                    ...
                    if cpu_key in self.results and f"{model_name}_cpu" in self.results[cpu_key]:
                        cpu_speed = self.results[cpu_key][f"{model_name}_cpu"]['predictions_per_second']
                        cpu_speeds.append(cpu_speed)
                    
                    if gpu_key in self.results and f"{model_name}_cuda" in self.results[gpu_key]:
                        gpu_speed = self.results[gpu_key][f"{model_name}_cuda"]['predictions_per_second']
                ...
                if cpu_speeds and gpu_speeds:
                    ...
                    plt.bar(x - width/2, cpu_speeds, width, label=f'{model_name} CPU', alpha=0.8)
                    plt.bar(x + width/2, gpu_speeds, width, label=f'{model_name} GPU', alpha=0.8)
            ...
            for batch_size in self.batch_sizes:
                ...
                for model_name in ['pytorch_nn', 'optimized_nn']:
                    ...
                    if (cpu_key in self.results and cpu_model_key in self.results[cpu_key] and
                        gpu_key in self.results and gpu_model_key in self.results[gpu_key]):
                        
                        cpu_speed = self.results[cpu_key][cpu_model_key]['predictions_per_second']
                        gpu_speed = self.results[gpu_key][gpu_model_key]['predictions_per_second']
            ...
            if speedup_by_batch:
                ...
                for i, (batch_size, speedups) in enumerate(speedup_by_batch.items()):
                    for model_name, speedup in speedups:
                        ...
                        plt.bar(i + 0.2 if 'pytorch' in model_name else i - 0.2, 
                            speedup, 
                            width=0.4, 
                            color=color,
                            alpha=0.7,
                ...
                plt.axhline(y=1, color='black', linestyle='--', alpha=0.5, label='Baseline (1x)')
                plt.xlabel('Batch Size')
            ...
            for model_name in self.models.keys():
                ...
                for batch_size in self.batch_sizes:
                    ...
                    if batch_key in self.results and model_name in self.results[batch_key]:
            ...
            for bar, speed in zip(bars1, avg_speeds):
                plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 10, 
                        f'{speed:.0f}', ha='center', fontsize=9)
            ...
            for bar, time_val in zip(bars2, avg_times):
                plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.1, 
                        f'{time_val:.2f}', ha='center', fontsize=9)
            ...
            for bar, size in zip(bars3, model_sizes):
                plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.1, 
                        f'{size:.2f}', ha='center', fontsize=9)
            ...
            if speedup_data:
                ...
                if comparison_names:
                    ...
                    bars4 = plt.bar(comparison_names, speedup_values, color=colors_comp, alpha=0.6)
                    plt.axhline(y=1, color='black', linestyle='--', alpha=0.5, label='Baseline (1x)')
            ...
            plt.tight_layout()
            plt.savefig('models/performance_comparison_final.png', dpi=150, bbox_inches='tight')
            plt.close()
            ...
            for model_name in self.models.keys():
                ...
                for batch_size in self.batch_sizes:
                    batch_key = f'batch_{batch_size}'
                    if batch_key in self.results and model_name in self.results[batch_key]:
                        speed = self.results[batch_key][model_name]['predictions_per_second']
                        if speed > 0:
                            speeds_by_batch.append(speed)
                            batch_labels.append(str(batch_size))
                
                if speeds_by_batch:
                    plt.plot(batch_labels, speeds_by_batch, 'o-', label=model_name.replace('_', ' '))
            ...
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            plt.savefig('models/batch_size_comparison.png', dpi=150, bbox_inches='tight')
            plt.close()
    ...
    def generate_summary_report(self):
        ...
        for batch_key, batch_results in self.results.items():
            for model_key, metrics in batch_results.items():
                ...
                if speed > best_speed:
                    ...
                    best_batch_size = batch_key.replace('batch_', '').replace('_cpu', '')
        ...
        for base_name in base_model_names:
            ...
            for batch_size in self.batch_sizes:
                ...
                if batch_key in self.results:
                    ...
                    model_key = model_key_with_suffix if model_key_with_suffix in self.results[batch_key] else base_name
        ...
        if 'onnx_nn' in performance_summary and 'pytorch_nn' in performance_summary:
            ...
            if pytorch_speed > 0:
                speedup = onnx_speed / pytorch_speed
                summary['recommendations'].insert(0, f"ONNX provides {speedup:.1f}x speedup over PyTorch")
        ...
        full_results = {
            ...
            'model_info': {
                model_name: {
                    ...
                    'size_mb': MODEL_PATHS.get(model_name, Path("")).stat().st_size / (1024 * 1024) 
                    if MODEL_PATHS.get(model_name, Path("")).exists() else 0
                }
                for model_name in self.models.keys()
            }
        }
    ...
    def print_final_report(self):
        ...
        print(f"   Average Speed: {summary['best_model']['avg_speed']:.1f} predictions/second")
        ...
        for model_name, metrics in summary['performance_summary'].items():
            ...
            print(f"{display_name:25} {metrics['avg_speed']:15.1f} {metrics['avg_time_ms']:12.3f} {metrics['model_size_mb']:10.2f}")
...
def main():
    ...
    parser.add_argument("--batch-sizes", type=str, default="1,4,16,64",
                       help="Comma-separated list of batch sizes")
    parser.add_argument("--simple", action="store_true",
                       help="Run simplified benchmark (faster)")
